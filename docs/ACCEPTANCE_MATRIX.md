...
| D14 | SIGTERM then forced stop during operation | Recovery manifest and pending work retained; fresh process can reconcile | E9 | `loop/interfaces/cli.py` (daemon) + `loop/runtime/service.py` | `test_remaining.py::test_d14_*`, `test_release_e2e.py::test_daemon_*`, `test_operations_r2.py::test_sigterm_releases_the_lease_and_a_forced_kill_does_not`, `test_operations_r2.py::test_pending_work_survives_a_forced_kill_and_a_fresh_process_reclaims_it` | verified |
...
| ID | Given / action | Required result | Milestone | Implementation | Test | Status |
|---|---|---|---|---|---|---|
| A01 | Prepare a trip using independent calendar/weather/knowledge reads | Bounded parallel assignments with one Coordinator, merged result and notification | C1 | `loop/runtime/planner.py`, `loop/runtime/authority.py` | `test_planner_authority.py::test_a01_*`, `test_coordinator_parallel_r4.py::test_independent_reads_all_execute_and_merge_into_one_result`, `test_coordinator_parallel_r4.py::test_parallel_reads_share_one_budget`, `test_coordinator_parallel_r4.py::test_a_failing_read_does_not_lose_the_others`, `test_coordinator_parallel_r4.py::test_a_cancelled_root_stops_every_remaining_read` | verified |
...
| A08 | Private raw note → wiki → retrieved answer → task → summary | local_only survives every derivative and team hand-off | C2 | `loop/services/knowledge.py + loop/vault/search.py + loop/core/privacy.py` | `test_planner_authority.py::test_a08_*`, `test_privacy_chain_r3.py::test_a_private_capture_is_persisted_as_local_only`, `test_privacy_chain_r3.py::test_a_local_only_request_never_reaches_the_cloud_when_local_fails` | verified |
...
| P09 | Meeting rescheduled or cancelled | Old pending reminders cancelled; updated occurrence uses new version | D4 | `loop/capabilities/calendar/service.py` | `test_routines_notify.py::test_p09_*`, `test_calendar_r4.py::test_a_rescheduled_meeting_cancels_its_stale_reminders`, `test_calendar_r4.py::test_a_meeting_that_disappears_cancels_its_reminders` | verified |
| P10 | One of two calendar providers fails | Successful results retained; failing provider marked unavailable, not free diary | D1/D3 | `loop/capabilities/calendar/service.py + adapters.py` | `test_observations_health.py::test_p10_*`, `test_calendar_r4.py::test_a_working_provider_is_kept_when_the_other_fails`, `test_calendar_r4.py::test_the_working_provider_is_kept_whichever_order_they_are_read_in` | verified |
| P11 | Repeated provider errors in same outage | One standalone outage notice, status/digest updates thereafter | D1/D3 | `loop/capabilities/calendar/service.py` | `test_observations_health.py::test_p11_*`, `test_calendar_r4.py::test_a_repeated_outage_is_reported_once`, `test_calendar_r4.py::test_the_outage_record_survives_a_restart` | verified |
...
| TR15 | Explicit monitor request with selected option, then restart | Durable scoped checkpoints/events until trip end; already-past and coincident checkpo… | E3 | `loop/runtime/trip_worker.py + loop/runtime/trip_monitor.py` | `test_travel.py::test_tr15_*`, `test_trip_monitor_e2e.py`, `test_trip_worker_r4.py::test_a_queued_check_is_claimed_and_produces_a_result`, `test_trip_worker_r4.py::test_the_check_survives_a_restart_and_runs_in_the_new_process` | verified |
| TR16 | Repeated closure/disruption events and no-change checks | One material-change notice with before/after evidence and alternatives; no-change che… | E3 | `loop/runtime/trip_worker.py + loop/capabilities/travel/monitor.py` | `test_travel.py::test_tr16_*`, `test_trip_worker_r4.py::test_a_material_change_produces_one_notice_with_before_and_after`, `test_trip_worker_r4.py::test_a_no_change_check_invokes_no_model` | verified |
| TR17 | Changed cost quote has different party/fees/date basis | No misleading price-change alert; comparison blocked or labelled non-comparable | E3 | `loop/runtime/trip_worker.py + loop/capabilities/travel/monitor.py` | `test_travel.py::test_tr17_*`, `test_trip_worker_r4.py::test_a_quote_on_a_different_basis_is_not_reported_as_a_price_change` | verified |
| TR18 | New proposed itinerary moves a confirmed booking | Booking remains a fixed anchor; no remote change; separate proposal/authority needed | E3 | `loop/capabilities/travel/schedule.py`, `loop/capabilities/travel/trip.py` | `test_travel.py::test_tr18_*`, `test_coordinator_parallel_r4.py::test_a_confirmed_booking_is_a_fixed_anchor`, `test_coordinator_parallel_r4.py::test_an_unbooked_or_unpinned_segment_is_not_an_anchor`, `test_coordinator_parallel_r4.py::test_moving_a_confirmed_booking_requires_its_own_authority`, `test_coordinator_parallel_r4.py::test_no_shipped_capability_can_make_a_remote_travel_change` | verified |
| TR19 | Stop monitoring, cancel trip, or select a new revision | Correct future triggers/subscriptions updated atomically; pending obsolete alerts can… | E3 | `loop/runtime/trip_worker.py + loop/runtime/trip_monitor.py` | `test_travel.py::test_tr19_*`, `test_trip_monitor_e2e.py`, `test_trip_worker_r4.py::test_a_check_queued_before_cancellation_does_not_run`, `test_trip_worker_r4.py::test_a_check_for_a_superseded_revision_does_not_run` | verified |
...
| EX14 | Weather/travel loaded as packs plus simple checklist extension | All share registry/lifecycle/executor; large domain schemas do not become required si… | E9 | `packs/weather + packs/travel + packs/checklist` via `loop/capabilities/registry.py` | `test_remaining.py::test_ex14_*`, `test_packs_ex14_r3.py::test_the_same_enable_disable_lifecycle_serves_all_of_them`, `test_packs_ex14_r3.py::test_the_checklist_pack_carries_none_of_the_large_packs_boilerplate` | verified |
...
| O05 | Run with wrong Python through setup | Clear version diagnostic before arbitrary import traceback | E5 | `loop/interfaces/cli.py` (guard above every heavy import) + `loop/ops/doctor.py` | `test_ops_and_packaging.py::test_o05_*`, `test_operations_r2.py::test_an_old_interpreter_is_refused_before_the_heavy_imports` | verified |
...
| O10 | Rebuild indexes after corruption or deletion | Same searchable authorized facts; original state untouched | E7 | `loop/services/maintenance.py` + `loop/vault/search.py` | `test_ops_and_packaging.py::test_o10_*`, `test_operations_r2.py::test_a_deleted_index_is_rebuilt_with_the_same_searchable_facts`, `test_operations_r2.py::test_a_rebuild_never_writes_to_the_sources` | verified |
| O11 | Retention maintenance | Expired content/derivatives removed, active work pinned, ordinary vault evidence pres… | E7 | `loop/services/maintenance.py` + `loop/ops/retention.py` | `test_ops_and_packaging.py::test_o11_*`, `test_operations_r2.py::test_expired_operational_state_is_removed_from_the_real_store`, `test_operations_r2.py::test_ordinary_vault_evidence_is_never_touched_by_retention` | verified |
| O12 | 500-source synthetic load and timer workload | Report persistence/claim latency, hardware, queue depth; compare main §9 targets | E7 | `loop/services/maintenance.py` + `loop/ops/perf.py` | `test_ops_and_packaging.py::test_o12_*`, `test_operations_r2.py::test_the_load_run_persists_real_rows_and_reports_percentiles` | verified |
| O13 | Current service halted/asleep | Status reports stale heartbeat and catch-up limits, not guaranteed continuous availab… | E5 | `loop/runtime/service.py` (persisted heartbeat) + `loop/ops/doctor.py` | `test_ops_and_packaging.py::test_o13_*`, `test_operations_r2.py::test_a_stale_heartbeat_is_reported_with_the_catch_up_limit`, `test_operations_r2.py::test_the_heartbeat_survives_a_restart` | verified |
